...
export default function MonthlyChart({ calc, baseline }: Props) {
  const data = PROJECTED_MONTHS.map((m) => {
    ...
    const expenses = (calc.cogs[m] || 0) + (calc.salary[m] || 0) + (calc.tax[m] || 0) + (calc.overhead[m] || 0);
    const cashFlow = calc.cashFlow[m] || 0;
  ...
  return (
    <div className="card p-5 h-full">
      <div className="flex items-center justify-between mb-3">
        <div>
          <h3 className="text-sm font-semibold text-ink-900">Monthly Revenue, Expenses & Cash Flow</h3>
        ...
        <div className="flex items-center gap-3 text-xs text-ink-500">
          ...
          <span className="flex items-center gap-1.5"><span className="h-2 w-3 rounded-sm bg-ink-300" /> Expenses</span>
          <span className="flex items-center gap-1.5"><span className="h-0.5 w-3 bg-good-500" /> Cash flow</span>
        </div>
      </div>
      <div className="h-72">
        <ResponsiveContainer>
          <ComposedChart data={data} margin={{ top: 8, right: 8, left: 8, bottom: 4 }}>
            ...
            <Bar dataKey="expenses" name="Expenses" fill="#d8d8df" radius={[6, 6, 0, 0]} />
            <Line
              type="monotone"
              dataKey="cashFlow"
              name="Cash flow"
